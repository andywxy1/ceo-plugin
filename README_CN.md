...
一个 Claude Code 插件，可协调 150+ 个专业 AI 智能体，覆盖工程、设计、营销、销售等多个领域。CEO 通过结构化需求发现、构建带依赖关系的执行计划，并以硬性门控、质量门控和状态报告来编排智能体团队。

## 包含内容

- **1 个技能** (`/ceo:ceo`) - 统筹全局的元编排器
- **152 个专业智能体**，覆盖 13 个领域（工程、设计、营销、销售、产品、项目管理、测试、支持、付费媒体、游戏开发、空间计算/XR、专项、战略）
...
运行 `/agents` 查看已加载的 152 个智能体，或运行 `/help` 确认 `ceo:ceo` 已列在可用技能中。
...
3. **预检阶段** - 以只读模式启动关键智能体，在正式执行前发现歧义（所有项目规模均强制执行）
...
| 领域 | 数量 | 示例 |
|------|------|------|
| 工程 | 23 | 后端架构师、前端开发者、DevOps、安全工程师、SRE |
...
| 专项 | 29 | MCP 构建者、工作流架构师、文档生成器、ZK 管家 |
| 战略 | 1 | 智能体编排器 |
